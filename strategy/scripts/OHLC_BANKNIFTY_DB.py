...
nifty_ohlc = get_index('NIFTY BANK')
niftybank = nifty_ohlc['IndexValue']
...
strikeRange = [str(i) for i in list(range(strkPrcCalc(niftylow, 100) - 400, strkPrcCalc(niftyhigh, 100) + 400,100))]
...
filtrdf1 = df[(df.Name == 'BANKNIFTY') & (df.Description.str.contains(f'BANKNIFTY21{monthly_exp[2:-4].upper()}')) ]
filtrdf2 = df[(df.Name == 'BANKNIFTY') & (df.Description.str.contains(f"BANKNIFTY21{(datetime.strftime(datetime.strptime(weekly_exp, '%d%b%Y'),'%#m%d'))}")) ]
...
skipped = []
db = sqlite3.connect(f'../ohlc/BANKNIFTY_{monthly_exp[2:-4].upper()}_OHLC.db')
cur = db.cursor()
for name,symbol in keyv.items():
    try:
        ...
        if dataresp != '':
            ...
            datadf.to_sql((f'BANKNIFTY_{datetime.now().strftime("%B").upper()}'),db,if_exists='append',index=False)
